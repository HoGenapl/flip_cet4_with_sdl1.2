...
time_t t;
char head_file[] = "./mp3/";
int loop_all_the_night = 0;//是否循环: 默认关闭
int display_status = 0;//显示状态: 0朝下 1朝右 2朝左
...
int play_and_show()
{
				...
				SDL_Surface* loop_sign = NULL;//循环标志
				if(loop_all_the_night)
				{
					loop_sign = TTF_RenderUTF8_Blended(font,"F",color2);
					apply_surface(310,0,loop_sign,screen,NULL);
				}
				else
				{
					loop_sign = TTF_RenderUTF8_Blended(font,"F",color3);
					apply_surface(310,0,loop_sign,screen,NULL);
				}
...
int fmn()
{
    ...
    while(quit == 0)
    {
        while(SDL_PollEvent(&event))
        {
			...
			else if((event.key.keysym.sym == 13) && event.key.type == SDL_KEYDOWN)
			{
				loop_all_the_night = loop_all_the_night ? 0:1;
				play_and_show();
			}
			//下一个单词
            ...
            {
				random_one();
				rotate_by_key(event.key.keysym.sym);
				play_and_show();
			}
			//播放音乐
...
int main(int argc,char* args[])
{
	printf("start:init()\n");
	init();//初始化
	SDL_WarpMouse(1000,1000);//把鼠标移动到屏幕外
	char numjoy[20];
    //初始化单词
